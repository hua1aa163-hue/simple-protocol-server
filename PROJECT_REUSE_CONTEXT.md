...
6. 支持严格的整文件夹串扰测试：设备确认当前测试完成后，等待 1 秒，再投放下一张并发送下一次测试命令。
7. 主界面与投影界面是两个可同时操作的非模态窗口。
8. 所有界面器件都应在 WinForms Designer 中创建和编辑；业务代码不负责动态拼装界面。
9. 代码应有面向新手的中文注释，并把协议、网络、投影系统调用与窗体业务分层。
...
8. 使用非模态 `_projectionForm.Show()` 打开投影窗体。
9. 把所有控件放在 `.Designer.cs` 中；业务 `.cs` 只编写事件和逻辑。
10. 在项目文件中为两个窗体保存 `SubType`、`DependentUpon` 和资源归属关系，确保 Visual Studio Designer 能识别。
...
7. 所有固定控件必须可在 WinForms Designer 中编辑。
8. 为新手添加中文注释，并补充与修改内容相称的自动测试。
9. 完成后执行 Release 编译和冒烟测试，报告实际结果。
```
