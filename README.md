...
发布目录中的 `AutoTestClient.exe` 可直接拷贝到测试机。此前已在真实 MRTEST 上完成黑白对比度和 FOV/9Point 弹窗、投图、BM_CLICK、Run→OK 与导出文件验收；`v0.1.1` 收紧监视生命周期并增加日志/计划列表回归测试。串扰及完整双屏现场组合仍按后续测试结果继续完善。
