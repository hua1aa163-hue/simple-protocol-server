...
namespace SimpleProtocolServer.DataProcessing;

/// <summary>一张投影图片完成设备测试的时间，用于和导出文件时间逐次对应。</summary>
internal sealed record CrosstalkTestRecord(
    int Sequence,
    string ImagePath,
    DateTime CompletedUtc);

/// <summary>开始测试前某个一级导出文件夹的状态，用来排除历史数据。</summary>
internal sealed record ExportFolderFingerprint(
    string? ExcelPath,
    long ExcelLength,
    long ExcelLastWriteUtcTicks);

/// <summary>MATLAB 等价计算后的矩阵与统计结果；内部数值仍是比例，例如 0.03 表示 3%。</summary>
internal sealed record CrosstalkCalculationResult(
...
internal sealed record CrosstalkProcessingResult(
...
internal static class CrosstalkDataProcessor
{
    ...
    /// </summary>
    internal static IReadOnlyDictionary<string, ExportFolderFingerprint> CaptureSnapshot(
        string sourceRoot)
    {
    ...
    /// </summary>
    internal static async Task<CrosstalkProcessingResult> ProcessCompletedTestAsync(
        string sourceRoot,
        string outputRoot,
    ...
    internal static double[,] MergeBrightnessColumns(IReadOnlyList<string> excelFiles)
    {
    ...
    /// </summary>
    internal static CrosstalkCalculationResult Calculate(double[,] mergedData)
    {
    ...
    /// </summary>
    internal static IReadOnlyList<ExportFolderData> SelectBestTimedWindow(
    ...
    internal static void WriteCrosstalkWorkbook(
        string path,
        CrosstalkCalculationResult calculation)
    {
    ...
    internal sealed record ExportFolderData(
